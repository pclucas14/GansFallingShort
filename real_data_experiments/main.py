            ...
            for split in ['valid','test']:
                ...
                with torch.no_grad():
                    ...
                    if split == 'valid':
                        ...
                        keep_training = curr_valid_loss <= best_valid # continue if loss went down
                        best_valid = min(best_valid,curr_valid_loss)
                    if split == 'test':
                        best_test = np.mean(losses_dev) if best_valid==curr_valid_loss else best_test

            if rlm and not keep_training:
                # note that we don't need to do the test loop for the iteration where the valid loss went up
                assert best_test != 1e5
                return best_test
